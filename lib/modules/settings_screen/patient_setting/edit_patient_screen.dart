...
class EditProfileScreenPatient extends StatelessWidget {
  const EditProfileScreenPatient({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocConsumer<PatientCubit,PatientStates>(
      listener: (context, state) {
        if(state is GetPatientDataSuccessState){backTo(context: context, widget: const SettingScreenPatient());}
      },
      builder: (context, state) {
        // instance of model
        var model = PatientCubit.get(context).patientUserModel;
        var cubit = PatientCubit.get(context);
        var coverImage = PatientCubit.get(context).coverImage;

        var profileImage = PatientCubit.get(context).profileImage;
        ...
        return Scaffold(
          ...
          body: model != null
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    if (state is GetPatientDataLoadingState)
                            ...
                            Expanded(
                                child: Column(
                                  children: [
                                    ...
                                    if (state is UpdateProfilePatientImageLoadingState)
                                      LinearProgressIndicator(),
                                    SizedBox(
                                      height: 10,
                                    ),
                            ...
                            Expanded(
                                child: Column(
                                  children: [
                                    ...
                                    if (state is UpdateCoverPatientImageLoadingState)
                                      LinearProgressIndicator(),
                                    SizedBox(
                                      height: 10,
                                    ),
